...
def update_access_time():
    ...
    doc_ref = db.collection('collections').document(collection_id).collection('sections').document(section_id)
    doc = doc_ref.get()
    if doc.exists:
        doc_ref.update({'data.last_accessed': firestore.SERVER_TIMESTAMP})
        return jsonify({'message': 'Access time updated successfully'})
    else:
...
def summarize_youtube():
    ...
    num_sentences = int(request.json.get('num_sentences', 5))
